...
- If a task was supplied, write its exact text and constraints to a UTF-8 file and pass its absolute path with `--task-file`. Preserve the file and returned task ID for retries. The starter persists and addresses it before launch; use that handoff instead of typing task text into the lead terminal or impersonating a job. Reuse the same file and task ID when retrying.
- Inspect the returned coordinator, job, task record, and health. Reuse the existing lead; wait for native readiness and inspect blocking login or trust prompts before claiming activation. Confirm acknowledgment in the persisted task record. A wake receipt alone is not acknowledgment. If `task.state` is `resume_required`, explain the interrupted task and inspect previous delivery/results before using `--resume-task` under the user's existing continuation authority. Completed tasks are never replayed; a deliberate new identical task needs a new `--task-id`. Without a supplied task, leave the ready lead waiting for the user.
