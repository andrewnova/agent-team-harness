...
The daemon exists to connect Codex and Claude through the durable mailbox: receipt ACKs, semantic ACKs, check-ins, replies, cockpit visibility, importable state, visible Claude wake-ups, and Codex wake payloads. If Claude should do work, use a mailbox-first command such as `plan claude` with the default adapter, `review request`, `channel steer`, or `mailbox send --to claude --kind request --reply-required`. For Claude-owned tasks, pair that durable request with the visible-Claude rule above. For Claude-to-Codex messages, the daemon must queue `.agent-team/comms/codex-wake/` payloads and invoke `AGENT_TEAM_CODEX_WAKE_COMMAND` when configured so Codex can receive real-time notice while the mailbox remains truth. Then keep Codex moving and later import or acknowledge Claude's mailbox reply.
