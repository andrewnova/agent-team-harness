...
The launcher injects the teammate boot contract into every Claude startup prompt, records a durable launch marker when the visible shell command starts, and instructs Claude to run `channel boot-ack` plus visibly ACK that mailbox is truth. Treat launch marker, boot ACK, endpoint readiness, and smoke readiness as separate facts.
