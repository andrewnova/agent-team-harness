...
- For critical Claude-owned steering, use `channel steer` instead of a raw live ask. It queues a reply-required Codex-to-Claude mailbox request first, then in default mode runs or observes a bounded daemon live wake for that exact mailbox message. Treat the returned `durable_ack.mailbox_message_id` as the durable mailbox handle, but do not treat `mcp_emitted`, receipt ACK, or mailbox queueing as Claude's semantic reply. If default steering returns `blocking_next_step`, follow it before saying Claude is working. Use `--no-live` or `--mailbox-only` only when the user explicitly wants quiet mailbox-only mode.
