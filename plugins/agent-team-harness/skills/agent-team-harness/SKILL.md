...
Use `channel steer` for critical steering where Claude must not miss the instruction. The durable mailbox request is queued first; default visible mode briefly waits for a semantic mailbox reply and succeeds only when Claude gives a semantic live answer or a semantic mailbox reply already exists. If the wake is merely queued/sent/pending or live delivery is unproven, the command exits nonzero and prints `blocking_next_step` with the request id, target when known, wake packet path when available, manual recovery command, and `await reply` command. A failed live-channel attempt is a transport problem, not message loss. `mcp_emitted` only proves the first-party MCP server wrote a notification frame; it is not Claude's semantic reply. The receiving daemon may send a quick `receipt_ack`, but the receiving model still owes a real mailbox reply for requests, `reply_required` messages, and messages explicitly marked as requiring a reply. Non-required notify/reply messages remain visible as advisory inbox items until read or acked.
