...
Hard transport rule: do not use raw `ask_claude`, raw live-channel asks, or `complete_channel_request` as the delegation path for planning, implementation, frontend work, review, refactor, or debugging tasks. Those synchronous channel paths are only for health checks, smoke tests, and low-level bridge diagnostics, and `channel ask` exits nonzero unless Claude returns a semantic answer. Real work for Claude must be represented in harness state and delivered through mailbox-backed CLI flows so Codex can keep working while Claude is busy.

Visible-Claude rule: for Claude-owned tasks, especially frontend/UI/UX work, Codex must start or attach a visible Claude Code teammate and deliver steering through `channel steer` with live delivery enabled by default. The user should be able to see Claude receive Codex's steering unless the user explicitly asks for quiet/mailbox-only mode. Default `channel steer` is visible-or-blocking: it queues the durable mailbox request first, immediately checks live delivery for that exact request, and returns `blocking_next_step` unless Claude gives a semantic live answer or a semantic mailbox reply already exists. First-party MCP queued/emitted means the wake moved; it does not mean Claude answered. Receipt ACKs, `wake_sent`, and `wake_sent_reply_pending` also do not count as success. If `blocking_next_step.kind` is `first_party_mcp_reply_missing`, debug the missing Claude-side call to the first-party channel `reply` tool, `agent_team_reply`, `agent_team_ack`, or an equivalent semantic mailbox reply; do not reframe the failure as the legacy channel CLI being the work bus. If the first-party MCP wake moved but no semantic reply lands and this is a human-time-critical handoff, run `channel steer --recover-visible` or the returned `visible_recovery_command` so a fresh visible Claude session receives the exact mailbox request in its startup prompt and must reply through first-party MCP/mailbox before Codex claims success. Do not use `--legacy-live-push`, `--no-live`, `--mailbox-only`, mailbox-only delegation, background-only launch, or hidden/PTY launch for Claude-owned work unless the user explicitly requested that mode. If visible attach/start or live delivery fails, treat that as a blocking harness setup problem for the Claude-owned task: run the repair/auth/endpoint diagnostics, report the exact blocker, and do not claim Claude is working or mark the task delegated until a visible Claude channel is healthy or the user explicitly overrides the visible requirement.

Codex App sandbox rule: live Claude checks need Claude auth files and localhost channel access. If `channel status`, `channel doctor`, `channel steer`, or `blocking_next_step.operator_hint` reports a loaded/recent endpoint with `fetch_failed`, do not conclude that Claude is unauthenticated or the endpoint is broken from that sandboxed result alone. Rerun the live channel diagnostic or steering smoke from a local-permission context, then repair auth/endpoint only if it still fails there.

The daemon exists to connect Codex and Claude through the durable mailbox: receipt ACKs, real replies, check-ins, cockpit visibility, importable state, visible Claude wake-ups, and Codex wake payloads. If Claude should do work, use a mailbox-first command such as `plan claude` with the default adapter, `review request`, `channel steer`, or `mailbox send --to claude --kind request --reply-required`; ordinary Codex-to-Claude `notify`/`checkin` traffic should still be written to the mailbox first and queued to `.agent-team/comms/claude-mcp/outbox.jsonl` for the first-party Claude MCP channel server. The legacy Claude channel CLI is not the normal wake path; use it only for startup, smoke, or explicit compatibility diagnostics. For Claude-owned tasks, pair that durable request with the visible-Claude rule above. For Claude-to-Codex messages, the daemon must queue `.agent-team/comms/codex-wake/` payloads and invoke the first available Codex wake adapter: explicit `AGENT_TEAM_CODEX_WAKE_COMMAND` first, then the installed `agent-team-codex-wake` command recorded by `agent-team codex mcp install`. The first-party `agent-team-codex-mcp` adapter is the Codex-side reader/reply surface for that stream: `agent-team codex mcp install`, `agent_team_codex_watch_mailbox`, ACK, reply, and task-open tools. Then keep Codex moving and later import or acknowledge Claude's mailbox reply.
...
2. If the Claude bridge is missing or stale, run `agent-team doctor --fix --target <session-name>` before startup. `doctor --fix` installs the harness-managed `claude-channel-cli` package, writes local wrappers, and attempts user-scope MCP setup; it is the supported repair path, not a separate user workflow.
3. Run `agent-team start --name <session-name> --daemon` unless the user explicitly disables the Claude connection or asks for deterministic/offline mode. If the desired Claude workspace is not the harness repo root, pass `--project-dir <project-root>`; use `--fresh-claude` only when you intentionally require a brand-new endpoint. `start --daemon` invokes Claude channel ensure, first prefers the remembered endpoint id from `.agent-team/comms/claude-channel/session.json` when the same Codex-thread `session_identity.thread_ref` and `project_dir` match, then falls back only to a same-project endpoint whose target/display name matches the requested session name. A new named workstream must launch a visible Claude Code teammate in a separate Terminal window instead of silently reusing or renaming unrelated same-project Claude history. Use `--reuse-claude` or the compatibility alias `--no-fresh-claude` only when the user explicitly wants loose same-project reuse. Display names are labels/fallbacks, not primary identity. Visible launches generate a per-launch Claude MCP config under `.agent-team/comms/claude-channel/mcp-configs/`, use a launch-scoped server name such as `agent-team-claude-launch-<launch-id>`, and pass it with `--mcp-config` so the first-party MCP server receives the exact launch id/session/project/harness context instead of relying on or being shadowed by global Claude config inheritance. `--fresh-claude` bypasses remembered reuse and must produce a genuinely new same-project endpoint; when it does not, inspect `fresh_start_no_new_endpoint`, `discovered.probe`, `fresh_launch_probe`, `endpoint_selection`, `launch_marker`, `mcp_start`, `mcp_init`, `boot_ack`, `startup_proof`, and `fallback_packet` instead of treating the launch as delegated. The launch marker proves the visible shell command ran; `mcp_start` proves Claude spawned the first-party MCP server process for the launch; `mcp_init` proves Claude completed MCP initialization for the launch when available; the boot ACK proves Claude ran `channel boot-ack` after reading the startup contract; `endpoint_selection` names the endpoint binding strategy; `startup_proof` counts duplicate proof rows while selecting the latest durable row; none replace endpoint/smoke readiness for live steering. If the launch marker is recorded but boot ACK is missing, run/copy `agent-team channel startup-packet --launch-id <launch-id> --text`; the packet is a first-class manual recovery path. If Claude pastes status/error text instead of running the ACK command, import it with `agent-team channel startup-import --launch-id <launch-id> --text <reply>` so the response lands as a durable Claude-to-Codex mailbox check-in/reply. Startup packet/import does not bypass mailbox, review, merge, proof, or done gates. A same-name endpoint from another `project_dir` is reported as `workspace_mismatch` unless `--allow-cross-project-reuse` is explicitly passed. Visible Terminal launch is the default; a configured Codex Terminal launcher must not hijack startup unless `--launch-mode codex-terminal` is explicitly requested. Codex Desktop's built-in Terminal can be opened manually, and Codex can read terminal output once a terminal is attached, but the exposed Codex tool surface does not provide a native open/write-terminal API. Do not rely on AppleScript or coordinate automation as the default. If the user explicitly asks for Codex Terminal with `--launch-mode codex-terminal`, require `AGENT_TEAM_CODEX_TERMINAL_LAUNCHER` or `--codex-terminal-launcher <path>`, report launcher failure honestly, and do not silently fall back. Failed Claude startup blocks `start` by default; use `--allow-degraded-claude` only for diagnostics or intentionally Codex-only work. Use approved channel mode by default; `--use-development-channel` is a diagnostics-only escape hatch because Claude may pause visibly on its development-channel confirmation prompt. Use `--no-ensure-claude` only for offline/deterministic runs, `--no-daemon` only for tests or diagnostics, `--launch-mode pty` for hidden receiver fallback, and `--launch-mode background` only for diagnostics/CI. Add `--smoke` when the next step requires a live Claude reply now, and treat a failed smoke as a real readiness failure.
4. Inspect the returned `blocked_next_step` and `claude_channel_startup` fields before assuming Claude is available.
   - If `blocked_next_step.blocking` is true, follow its `command` immediately before doing Claude-dependent planning, review, UI work, or unrelated parallel branches. Do not ask "Planning Mode or Dev Mode" while a startup blocker is present.
   - For `claude_auth_required`, run the harness auth helper (`channel auth login`, usually via the returned `blocked_next_step.command`) and then rerun `start --daemon --smoke`. If login requires user browser interaction, report that exact auth blocker instead of continuing as though Claude has confirmed anything.
   - For missing bridge/CLI blockers, run `doctor --fix --target <session-name>` or the returned repair command before retrying startup.
   - For smoke failures, treat endpoint reachability as insufficient: use the returned status/repair command and require a real mailbox reply or successful smoke before claiming Claude confirmed work.
5. Inspect `agent-team config` when you need to confirm turbo parallel policy. Default should be `execution_profile: turbo_parallel`.
6. Run `agent-team daemon status` and `agent-team watch --once --no-live-channel` for a quick local cockpit snapshot, or `agent-team watch --target <session-name>` when live Claude status should be checked continuously. The cockpit scans receiver daemon health, realtime mailbox messages, pending receipt ACKs, pending real replies, Claude steering notices, Claude check-ins, Codex MCP adapter status, Codex wake stream state, Claude startup identity/probe state (`Claude startup:` including endpoint-selection, launch-marker, mcp-start, boot-ack, and duplicate-proof status), and the per-message timeline before surfacing Next Actions.
...
Operate the harness through the installed `agent-team` CLI. It is a global wrapper that always
runs the canonical harness regardless of your current directory, so you do not need to be in the
harness repo — run it from the project you are operating on:

```bash
agent-team <command>
```

State (`.agent-team`) is created under the directory you run from (or `--cwd <harness-root>`), so
run `agent-team` from your operated project. Do not invoke `agent-team` by a
relative path — that resolves against whatever repo you happen to be in.
...
```bash
agent-team db status
agent-team db rebuild
agent-team db query --sql "select * from tasks limit 5"
```
...
1. Initialize state with `agent-team init`.
...
- Requests, `reply_required` messages, and messages explicitly marked as needing a real reply must receive a mailbox `reply` from the receiving model that acknowledges the content, states what it will do next, and answers the question or names the blocker. Non-required `notify`, `checkin`, and human-relevant `reply` messages must still surface in cockpit/watch until read or acked, but they are advisory and do not block tasks.
...
- For critical Claude-owned steering, use `channel steer` instead of a raw live ask. It queues a reply-required Codex-to-Claude mailbox request first, then in default mode runs or observes a bounded daemon live wake for that exact mailbox message through the first-party Claude MCP outbox. Treat the returned `durable_ack.mailbox_message_id` as the durable mailbox handle, but do not treat `mcp_emitted`, receipt ACK, or mailbox queueing as Claude's semantic reply. If default steering returns `blocking_next_step`, follow it before saying Claude is working; for human-time-critical steering, use `--recover-visible` so the recovery launch and reply wait happen inside the same command. Use `--legacy-live-push`, `--no-live`, or `--mailbox-only` only when the user explicitly wants that mode.
...
```bash
agent-team refactor offer --goal <goal> --scope repo
agent-team refactor start <offer-id>
agent-team refactor import --run <run> --source codex --json codex-refactor.json
agent-team refactor import --run <run> --source claude --json claude-refactor.json
agent-team refactor compare --run <run>
agent-team refactor taskify --run <run> --create
```
...
```bash
agent-team self-heal context --limit 10
```
...
```bash
agent-team feedback record --goal <goal> --scope harness --text "<feedback>"
agent-team self-heal recommend --goal <goal> --recommendation "<recommendation>"
agent-team self-heal request-change --from claude --surface skill --request "<requested improvement>"
agent-team self-heal approve <recommendation-id> --note "<human approval>"
agent-team self-heal mark-applied <recommendation-id> --note "<what changed>" --evidence "<tests/proof>"
```
...
```bash
agent-team worktree create <task>
agent-team worktree status <task>
agent-team worktree snapshot <task> --message "Task snapshot"
agent-team worktree merge <task>
agent-team merge <task> --strategy worktree
```
...
```bash
agent-team codex-subagents import --json codex-subagents.json
agent-team codex-subagents list --task <task>
agent-team codex-subagents show <import-id>
```
...
```bash
agent-team agent-teams import --json agent-teams.json
agent-team agent-teams list --task <task>
agent-team agent-teams show <import-id>
```
...
```bash
agent-team moa record --json moa-council.json
agent-team moa list --scope task --subject <task>
agent-team moa show <council-id>
```
...
```bash
agent-team daemon start --roles codex,claude --include-existing
agent-team daemon status
agent-team watch --target codex-agent-team-harness
...
```bash
agent-team mailbox watch --to codex --unacked --interval-ms 1000
agent-team mailbox send --from claude --to codex --kind checkin --task T-000001 --subject "Still working" --body "Waiting on frontend Agent Teams."
node /abs/path/to/agent-team/src/cli.js --cwd /abs/path/to/harness mailbox send-batch --json claude-replies.json
agent-team channel dispatch --kind review --task T-000001 --prompt "Review this without blocking Codex."
agent-team channel steer --kind ui_direction --task T-000001 --goal G-000001 --subject "ACK REQUIRED" --file path/to/instruction.md
agent-team mailbox list --kind receipt_ack --limit 10
agent-team mailbox send --from claude --to codex --kind reply --in-reply-to msg_... --request-id req_... --subject "ACK: received" --body "ACK: received. I will do X next. Answer/blocker: ..."
```

Use `channel steer` for critical steering where Claude must not miss the instruction. The durable mailbox request is queued first; default visible mode briefly waits for a semantic mailbox reply and succeeds only when Claude gives a semantic live answer or a semantic mailbox reply already exists. If the first-party MCP wake is merely queued/emitted or live delivery is otherwise unproven, the command exits nonzero and prints `blocking_next_step` with the request id, target when known, manual recovery command, `visible_recovery_command`, and `await reply` command. `mcp_emitted` proves the first-party MCP server wrote a notification frame; it is not Claude's semantic reply. The missing success condition is Claude calling the first-party channel `reply` tool, `agent_team_reply`, `agent_team_ack`, or sending an equivalent semantic mailbox reply. If first-party MCP has emitted but Claude still has not replied, run `channel steer --recover-visible` for one-command recovery or run `visible_recovery_command` before declaring the handoff blocked. Do not try raw legacy channel asks; legacy channel details are compatibility diagnostics only and require explicit `--legacy-live-push` or `AGENT_TEAM_DAEMON_LEGACY_LIVE_PUSH=1`. The receiving daemon may send a quick `receipt_ack`, but the receiving model still owes a real mailbox reply for requests, `reply_required` messages, and messages explicitly marked as requiring a reply. Non-required notify/reply messages remain visible as advisory inbox items until read or acked.

If `blocking_next_step.operator_hint.kind` is `rerun_live_channel_with_local_permissions`, preserve the mailbox request id and rerun the live diagnostic/steer command with local auth/loopback permissions before opening another Claude or declaring auth broken.
...
The launcher injects the teammate boot contract into every Claude startup prompt, records a durable launch marker when the visible shell command starts, writes a per-launch `--mcp-config`, passes launch context into the Claude/MCP environment, sends an initial user prompt instructing Claude to run `channel boot-ack`, records first-party MCP process start when Claude spawns the server, and records first-party MCP init when the MCP server initializes. Treat endpoint selection, launch marker, MCP process start, MCP init, boot ACK, endpoint readiness, smoke readiness, and duplicate startup-proof rows as separate facts. If boot ACK is missing after a visible launch, use `channel startup-packet --launch-id <launch-id> --text` as the first-class copy/paste recovery path, then `channel startup-import --launch-id <launch-id> --text <reply>` when Claude pastes status/error text back.
...
agent-team channel mcp status
agent-team doctor --fix --target codex-agent-team-harness
agent-team channel ensure --name codex-agent-team-harness
agent-team channel ensure --name my-project --project-dir /path/to/project
agent-team channel ensure --name my-project --project-dir /path/to/project --fresh-claude
agent-team channel ensure --name my-project --project-dir /path/to/project --reuse-claude
```

Probe before asking:

```bash
agent-team channel doctor --fix --target codex-agent-team-harness
agent-team channel status --target codex-agent-team-harness
```
...
agent-team channel auth login
```

Send bounded synchronous live asks through the harness only when a reply-required smoke/proof check is explicitly needed:

```bash
agent-team channel ask --target codex-agent-team-harness --task T-000001 --kind review --timeout-ms 60000 --prompt "From Codex: review this. Reply through complete_channel_request."
```
...
npm test
cd ..
agent-team verify final   # when canonical tasks exist
```
