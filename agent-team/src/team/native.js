...
function buildNativeCommand(root, job, options = {}) {
  ...
  const instructions = [
    ...
    "Clean up only processes you started and can identify by their recorded PID and process start time. Never use broad process-name or command-pattern kills such as pkill -f or killall: other jobs and the user's tools may run the same program. Stop auxiliary servers and probes through their owned process handles; if ownership is uncertain, report it to the lead instead of killing matches.",
