...
Every job is a native session and can run its own child agents. Astra jobs use Codex multi-agent at high effort. Fable jobs use Claude Code Agent Teams, with every child kept on the parent's model. These defaults ship with the harness in a versioned config file and are applied per launch; your global Codex and Claude settings are not edited.
