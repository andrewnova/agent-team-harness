...
- Makes `channel steer` visible-or-blocking by default, with an optional visible recovery launch when Claude does not send the required mailbox reply.
...
The repo includes an experimental first-party Claude MCP server at `agent-team-claude-mcp`. It declares the Agent Team Claude Channel, watches `.agent-team/comms/claude-mcp/outbox.jsonl`, emits queued `notifications/claude/channel` wake-ups with protocol-compatible `sender`, `reply_required: "true" | "false"`, `received_at`, and native `chat_id`, `message_id`, `user`, and `ts` channel metadata, exposes the conventional channel `reply` tool plus mailbox-backed ACK/reply/check-in/status/task tools, and writes Claude responses through the same durable mailbox as the CLI.
...
This is the migration target for replacing the managed `claude-channel-cli` bridge on the normal path. The daemon now writes the first-party outbox first and treats the first-party MCP notification plus mailbox reply tools as the teammate lane. The legacy bridge remains only a startup/smoke/explicit-compatibility path and should not be diagnosed as the source of truth when first-party MCP has queued or emitted the notification.
